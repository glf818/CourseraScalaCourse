...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] = Encoder.fromFunction(n => Json.Str(n)) // TODO Implement the `Encoder[String]` instance
  /** An encoder for `Boolean` values */
  // TODO Define an implicit value of type `Encoder[Boolean]`
  implicit val booleanEncoder: Encoder[Boolean] = Encoder.fromFunction(n => Json.Bool(n))
  /**
...
trait DecoderInstances {
  ...
  implicit val booDecoder: Decoder[Boolean] =
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) =>  if (items.forall(x => decoder.decode(x) match
      {case Some(x)=> true
        case None => false })) Some(items.map(decoder.decode(_).get)) else None
    }
    ...
    new Decoder[A] {
      def decode(data: Json): Option[A] = data match {
        case Json.Obj(fields) => fields.get(name) match {
          case Some(x) => decoder.decode(x)
          case _ => None
        }
        case _ => None
      }
    }
    //Decoder.fromPartialFunction { case Json.Obj(fields) => decoder.decode(fields(name)).get }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    //( Decoder.strDecoder zip Decoder.intDecoder ).transform[Person](x => Person(x._1, x._2))
...
trait ContactsCodecs {
  ...
  /*Encoder.fromFunction(p =>
    Json.Obj(Map("people" -> Encoder.listEncoder[Person].encode(p.people))))*/
  ObjectEncoder.field[List[Person]]("people").transform[Contacts](p => p.people)

  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform[Contacts](listPerson => Contacts(people=listPerson))
    /*Decoder.fromPartialFunction {
    case Json.Obj(fields) => Contacts(fields("people").decodeAs[List[Person]].get)
  }*/
}
