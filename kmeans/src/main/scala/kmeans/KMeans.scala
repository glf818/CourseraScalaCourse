...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    var grpPt1 = points.groupBy(x => findClosest(x, means))
    for(extraPt <- means){
      if(!grpPt1.contains(extraPt)) grpPt1 = grpPt1 + (extraPt -> Seq[Point]().to(mutable.ArrayBuffer))
    }
    grpPt1
  }
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    var grpPt1 = points.groupBy(x => findClosest(x, means))
    for(extraPt <- means){
      if(!grpPt1.contains(extraPt)) grpPt1 = grpPt1 + (extraPt -> ParSeq[Point]())
    }
    grpPt1
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    var dist = 0.0;
    oldMeans.zip(newMeans).foreach { x => {dist += x._1.squareDistance(x._2)} }
    if (dist <= eta) true else false
  }

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean = {
    val dist = oldMeans.zip(newMeans).map{ case (oldP, newP) => oldP.squareDistance(newP)}.fold(0.0)(_+_)
    if (dist <= eta) true else false
  }
