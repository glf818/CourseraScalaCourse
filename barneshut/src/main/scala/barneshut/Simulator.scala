...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    newB.minX = math.min(boundaries.minX, body.x)
    newB.maxX = math.max(boundaries.maxX, body.x)
    newB.minY = math.min(boundaries.minY, body.y)
    newB.maxY = math.max(boundaries.minY, body.y)
    newB
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val newB = new Boundaries()
    newB.minX = math.min(a.minX, b.minX)
    newB.maxX = math.max(a.maxX, b.maxX)
    newB.minY = math.min(a.minY, b.minY)
    newB.maxY = math.max(a.maxY, b.maxY)
    newB
  }
  ...
  def updateBodies(bodies: coll.Seq[Body], quad: Quad): coll.Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(_ updated quad).aggregate(Seq[Body]())(_ :+ _, _ ++ _)
  }
