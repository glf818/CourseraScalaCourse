...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = (b.b1.row == goal.row) && (b.b2.row == goal.row) && (b.b1.col == goal.col) && (b.b2.col == goal.col)
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val m =
      for {x <- b.legalNeighbors} yield (x._1, x._2 :: history)
    LazyList(m: _*)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val m =
      for {
        init <- initial
        initLN <- init._1.legalNeighbors
        if !(explored contains initLN._1)
      } yield (initLN._1, initLN._2 :: init._2)
    m.to(LazyList)
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val m = for {
      nb <- startBlock.legalNeighbors
    } yield (nb._1, nb._2 :: Nil)
    m.to(LazyList)
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    ...
    def auxLoop(initPaths: LazyList[(Block, List[Move])], initExplored: Set[Block]): LazyList[(Block, List[Move])] = {
      val initPassed: LazyList[(Block, List[Move])] = initPaths.filter(x => (x._1.b1 == goal) && (x._1.b2 == goal))
      val initNotPassed = initPaths.filter(x => ((x._1.b1 != goal) || (x._1.b2 != goal)) && x._1.isLegal)
      val exploredBlocks = initExplored ++ initNotPassed.toList.map(_._1).toSet
      initPassed #::: auxLoop(from(initNotPassed, exploredBlocks), exploredBlocks)
    }

    auxLoop(pathsFromStart, Set(startBlock))
  }
