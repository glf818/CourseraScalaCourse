...
trait BinomialHeap extends Heap {
  ...
  case class Node(x: A, r: Rank, c: List[Node]) {
    override def toString: String = c match  {
      case Nil => "{ x: " + x.toString + ", r: " + r.toString + "}"  //", c: Nil}"
      case _ => "{ x: " + x.toString + ", r: " + r.toString +/* + ", c: " + c.length.toString + */ "}"
    }
  }
  override type H = List[Node]
  def depth(t: Node): Int = t.c match {
    case Nil => 0
    case x: List[Node] => x.map(depth).max + 1
  }

  def root(t: Node) = t.x
  def rank(t: Node) = t.r
  def link(t1: Node, t2: Node): Node = // t1.r == t2.r
    if (ord.lteq(t1.x, t2.x)) Node(t1.x, t1.r + 1, t2 :: t1.c) else Node(t2.x, t2.r + 1, t1 :: t2.c)
  def ins(t: Node, ts: H): H = ts match {
    case Nil => List(t)
    case tp :: ts => // t.r <= tp.r
      if (t.r < tp.r) t :: tp :: ts else {//println("linking " + t.toString + " , " +  tp.toString)
        ins(link(t, tp), ts)}
  }
  ...
  override def insert(x: A, ts: H) =
    { //println("insert " + x.toString)
      ins(Node(x, 0, Nil), ts)}
...
trait Bogus2BinomialHeap extends BinomialHeap {
  override  def link(t1: Node, t2: Node): Node = // t1.r == t2.r
    if (!ord.lteq(t1.x, t2.x)) Node(t1.x, t1.r + 1, t2 :: t1.c) else Node(t2.x, t2.r + 1, t1 :: t2.c)
}
